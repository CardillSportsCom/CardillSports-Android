...
public class TableUtils {
    ...
    public static List<List<Stat>> generateTableCellList(List<Player> teamOne, List<Player> teamTwo) {
        ...
        for (Player player : teamOne) {
            ...
            statList.add(new Stat(GameStatType.MAKE_ONE_POINT, player.getOnePointFieldGoalMade(), true));
            statList.add(new Stat(GameStatType.MAKE_TWO_POINT, player.getTwoPointFieldGoalMade(), true));
        ...
        for (Player player : teamTwo) {
            List<Stat> statList = new ArrayList<>(8);
            statList.add(new Stat(GameStatType.MAKE_ONE_POINT, player.getOnePointFieldGoalMade(), false));
            statList.add(new Stat(GameStatType.MAKE_TWO_POINT, player.getTwoPointFieldGoalMade(), false));
